...
const contactController = {
  async submit(req, res, next) {
    try {
      ...
      // Send notification to you (non-blocking — don't fail the endpoint if email is misconfigured)
      try {
        const adminEmail = process.env.ADMIN_EMAIL || 'admin@example.com';
        const html = `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #0c0c0e; color: #fff;">
        ...
        console.error('[CONTACT] Email send failed (saved to DB):', emailErr.message);
      }

      // Send confirmation to submitter (also non-blocking)
      try {
        await sendEmail({
          to: email,
          ...
          html: `<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 500px; margin: 0 auto; padding: 32px; background: #0c0c0e; color: #fff;">
            ...
            <p style="color: #A1A1AA; font-size: 14px; line-height: 1.7;">
              Thanks, ${escapeHtml(name)}. We received your brief for ${escapeHtml(company)}.
              A partner will reply from admin@example.com within two business days.
              If the work is outside our range, we will say so plainly.
            </p>
